...
import jp.opap.data.yaml.Leaf.{EmptyNode, StringNode}
...
object Manifest {
  ...
  def toWarning(exception: YamlException): GlobalWarning = {
    def nonMapping(node: Node) = GlobalWarning(UUID.randomUUID(), s"${node.location}: ${WARNING_NODE_TYPE_INVALID.format(node.getClass.getSimpleName)}", None)
  ...
  def toWarning(e: GlobalException): GlobalWarning = {
