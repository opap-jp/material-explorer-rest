...
**開発用**（`docker-compose.yaml`）は、`db` コンテナのみを起動します。後述する開発環境を整えることで、テスト用の構成に対して頻繁な Docker イメージのビルドを必要としないため、効率的な開発ができます。
...
1. `npm run start` で、開発用ウェブサーバーが起動します。[http://localhost:8000](http://localhost:8000) が自動的に開かれます。
